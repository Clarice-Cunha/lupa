...
function traduzirErro(mensagem: string): string {
  ...
  if (m.includes("413") || m.includes("maior que o limite")) {
...
export default function Home() {
  ...
  async function aoEnviarImagem(evento: React.FormEvent) {
    ...
    setEstado("carregando");
    try {
      const dados = await analisarImagem(imagemArquivo);
...
function FormularioImagem({ arquivo, setArquivo, aoEnviar, erro }: FormularioImagemProps) {
  const inputRef = useRef<HTMLInputElement>(null);
  const [arrastando, setArrastando] = useState(false);
  ...
  return (
    <section className="animate-fade-in-up rounded-3xl border border-slate-200/60 bg-white/80 p-6 shadow-xl shadow-indigo-100/50 backdrop-blur-sm sm:p-8">
      <form onSubmit={aoEnviar} className="space-y-4">
        <label className="flex items-center gap-2 text-sm font-medium text-slate-700">
          <Sparkles className="h-4 w-4 text-indigo-500" />
          Envie uma imagem para ver seus metadados
        </label>
        ...
        {!arquivo ? (
          ...
          >
            <FileImage className="h-10 w-10 text-indigo-500" strokeWidth={1.5} />
            <p className="font-medium text-slate-700">
              Arraste uma imagem aqui ou clique para escolher
            </p>
            <p className="text-xs text-slate-500">
              Formatos: JPG, PNG, WEBP, GIF, BMP, TIFF — até 20 MB
            </p>
            <input
              ref={inputRef}
              type="file"
          ...
          <div className="flex items-center gap-3 rounded-2xl border border-indigo-200 bg-indigo-50/50 p-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs text-slate-500">{formatarTamanho(arquivo.size)}</p>
            </div>
            <button
              type="button"
              onClick={() => setArquivo(null)}
      ...
      <div className="mt-6 flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
        ...
        <p>
          ...
          pela câmera ou dispositivo quando a foto é tirada: data, horário, modelo da câmera,
          software de edição e até as coordenadas GPS. Esses dados ajudam a verificar a
...
function ResultadoImagem({ dados, aoReiniciar }: { dados: RespostaImagem; aoReiniciar: () => void }) {
  ...
  return (
    <section aria-live="polite" aria-label="Resultado da análise de imagem" className="space-y-5">
      {/* Cartão principal */}
      ...
      {dados.tem_exif && (
        <div
          className="animate-fade-in-up rounded-3xl border border-slate-200/60 bg-white/80 p-6 shadow-lg backdrop-blur-sm"
          style={{ animationDelay: "0.1s" }}
        >
      ...
      {/* Busca reversa */}
      <div
        className="animate-fade-in-up rounded-3xl border border-emerald-100 bg-gradient-to-br from-emerald-50 to-white p-6 shadow-lg"
        style={{ animationDelay: "0.3s" }}
      >
